$(function() {
	...
	if ($(".counter-item [data-to]").length > 0) {
		$(".counter-item [data-to]").each(
				function() {
					var stat_item = $(this), offset = stat_item.offset().top;
	...
	var shuffleme = (function($) {
		...
		$filterOptions = $('.portfolio-sorting li'), // locate the filter
														// categories
		...
		init = function() {
			...
			$("#loadMore").on(
					...
					function(e) {
						e.preventDefault();
						$("#grid .col-md-4:hidden").slice(0, 4).fadeIn().each(
								function() {
									$grid.shuffle('appended', $(this));
								});

						if ($("#grid .col-md-4:hidden").length == 0) {
							$("#loadMore").addClass("disabled").html(
									"No more to Load");
						}
					});
		...
		setupFilters = function() {
			...
			$btns.on('click',
					function(e) {
						e.preventDefault();
						var $this = $(this), isActive = $this
								.hasClass('active'), group = isActive ? 'all'
								: $this.data('group');
...
if ($('#map').length > 0) {
	...
	function init() {
		...
		var myLatlng = new google.maps.LatLng(40.69847032728747,
				-73.9514422416687);
